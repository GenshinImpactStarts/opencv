...
// {
//     int len = __riscv_vsetvlmax_e8m2();
//     vuint32m1_t result_v = __riscv_vmv_v_x_u32m1(0, 1);
//     const uchar* tab;
//     switch (cellSize)
//     {
//         case 1: tab = popCountTable; break;
//         case 2: tab = popCountTable2; break;
//         case 4: tab = popCountTable4; break;
//         default: return CV_HAL_ERROR_NOT_IMPLEMENTED;
//     }
//     for (; n >= len; n -= len, a += len)
//     {
//         auto v = __riscv_vle8_v_u8m2(a, len);
//         auto cnt = __riscv_vzext_vf2(__riscv_vluxei8(tab, v, len), len);
//         result_v = __riscv_vwredsumu(cnt, result_v, len);
//     }
//     for (; n > 0; n -= len, a += len)
//     {
//         len = __riscv_vsetvl_e8m2(n);
//         auto v = __riscv_vle8_v_u8m2(a, len);
//         auto cnt = __riscv_vzext_vf2(__riscv_vluxei8(tab, v, len), len);
//         result_v = __riscv_vwredsumu(cnt, result_v, len);
//     }
//     *result = static_cast<int>(__riscv_vmv_x(result_v));
...
template <typename CellType>
inline void normHammingCnt_m4(vuint8m4_t v, vbool1_t mask, size_t len_bool, size_t& result)
{
    auto v_bool0 = __riscv_vreinterpret_b1(__riscv_vget_u8m1(v, 0));
    auto v_bool1 = __riscv_vreinterpret_b1(__riscv_vget_u8m1(v, 1));
    auto v_bool2 = __riscv_vreinterpret_b1(__riscv_vget_u8m1(v, 2));
...
template <typename CellType>
inline void normHammingCnt_m2(vuint8m2_t v, vbool1_t mask, size_t len_bool, size_t& result)
{
    auto v_bool0 = __riscv_vreinterpret_b1(__riscv_vget_u8m1(v, 0));
    auto v_bool1 = __riscv_vreinterpret_b1(__riscv_vget_u8m1(v, 1));
    result += CellType::popcount(v_bool0, mask, len_bool);
