...
template <typename VEC_T> struct RVV_GetElemImpl;
template <typename VEC_T> struct RVV_GetLmulImpl;

template <typename VEC_T>
using RVV_GetElem = typename RVV_GetElemImpl<VEC_T>::type;

template <typename VEC_T>
using RVV_GetLmul = typename RVV_GetLmulImpl<VEC_T>::type;

template <typename VEC_T>
using RVV_FromVec = RVV<RVV_GetElem<VEC_T>, RVV_GetLmul<VEC_T>::value>;
...
#define HAL_RVV_DEFINE_ONE(ELEM_TYPE, VEC_TYPE, LMUL_TYPE, \
    ...
    {                                                      \
        return v;                                          \
                                                           ...
                                                           \
    template <>                                            \
    struct RVV_GetElemImpl<v##VEC_TYPE##LMUL##_t>          \
    {                                                      \
        using type = ELEM_TYPE;                            \
    };                                                     \
                                                           \
    template <>                                            \
    struct RVV_GetLmulImpl<v##VEC_TYPE##LMUL##_t>          \
    {                                                      \
        using type = std::integral_constant<RVV_LMUL, LMUL_TYPE>; \
    };
...
#define HAL_RVV_SEG(RVV_T, SEG_T, SEG, SEG_EEW, TYPE)                                        \
    ...
    inline SEG_T RVV_T::vload_sseg<SEG>(BoolType vm,                                         \
        ...
        return __riscv_vls##SEG_EEW##_v_##TYPE##_m(vm, ptr, unit, vl);                       \
                                                                                             ...
                                                                                             \
    template <>                                                                              \
    struct RVV_GetElemImpl<SEG_T> {using type = typename RVV_T::ElemType;};                  \
    template<>                                                                               \
    struct RVV_GetLmulImpl<SEG_T> {using type = std::integral_constant<RVV_LMUL, RVV_T::lmul>;};
